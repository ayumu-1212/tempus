...
export const RecordsList = forwardRef<RecordsListRef>((props, ref) => {
  ...
  const [editingRecord, setEditingRecord] = useState<RecordWithType | null>(
    null,
  );
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const response = await fetch(
        `/api/records/pdf?year=${year}&month=${month}`,
      );

      if (!response.ok) {
        const errorData: PdfError = await response.json();
        setPdfError(errorData.message || errorData.error);
        return;
      }
      ...
      a.href = url;
      a.download = `attendance_${year}_${month.toString().padStart(2, "0")}.pdf`;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">月次集計</h3>
          <button
            type="button"
