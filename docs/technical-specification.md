...
| type | TEXT | NOT NULL | 出勤: 'clock_in', 退勤: 'clock_out' |
...
**処理フロー**
1. 最新の打刻レコードを取得
2. 最新が `clock_in` → `clock_out` を記録
3. 最新が `clock_out` または存在しない → `clock_in` を記録
4. Discord通知を送信（環境変数で設定された場合）
...
**処理フロー**
1. 最新の打刻レコードを取得
2. 最新が `clock_in` → `clocked_in` を返す
3. 最新が `clock_out` または存在しない → `clocked_out` を返す
